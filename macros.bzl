...
def _convert_to_url(artifact):
    ...
    return [
        url_prefix % "libs-snapshots" + url_suffix,
...
def deprecated_alias(name, actual_targets, **kwargs):
    """Creates alias to one of more jvm targets with deprecation message

    Args:
      name: The name of the alias
      actual_targets: One or more jvm targets labels (scala_library / java_library)
      **kwargs: other attributes you may wish to add
    """
    native.java_import(
        name = name,
        deprecation = "Deprecated. Please one of the labels in %s" % actual_targets,
        jars = [],
        exports = actual_targets,
        **kwargs
    )
